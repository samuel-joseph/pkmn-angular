  ...
  toStore() {
    ...
    for (const myPokemon of this.myPokemons) {
      ...
      for (const move of myPokemon.moves) {
        console.log("What is move id ",move)
        console.log(move.id+"==="+this.moveListArr[0])
        if (this.moveListArr.includes(move.id)) {
          console.log("for loop move ", move)
          tempArrMoves.push(move)
        } else {
